    ...
    if (hdfu->wblock_num == 0U)
    {
      ...
      else if (hdfu->wlength == 5U)
      {
        ...
        else if (hdfu->buffer.d8[0] == DFU_CMD_ERASE)
        {
          ...
          // added code to set a variable when an erase is being done which is the first step in firmware update and set it if other state are not set
       	  printf("erase operation occured\n");
          if ((erase_operation ==0) && (end_of_download_operation==0))
           {
        	  printf("set variable erase_operation\n");
         	  erase_operation =1;
           }
        }
        else
...
static void DFU_Download(USBD_HandleTypeDef *pdev, USBD_SetupReqTypedef *req)
{
  ...
  else
  {
    ...
    if ((hdfu->dev_state == DFU_STATE_DNLOAD_IDLE) || (hdfu->dev_state == DFU_STATE_IDLE))
    {
     ...
     printf("End of DNLOAD operation\n");
  	 if ((erase_operation==1) && (end_of_download_operation ==0))
  	 {
  		 printf("set variable end_of_download_operation\n");
...
static void DFU_Leave(USBD_HandleTypeDef *pdev)
{
  ...
  else
  {
    ...
    /* Generate system reset to allow jumping to the user code */
	 ...
	 // test if an erase and  firmware upgrade  has been done, then do a swap because this indicates a firmware update
	 if ((erase_operation==1) && (end_of_download_operation==1))
 	  {
		 printf("Firmware has been updated and now we will swap banks to execute updated code\n");
		 erase_operation=0;
